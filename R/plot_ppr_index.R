...
plot_ppr_index <- function(scaled, epu, withLandings = F) {
  if (!withLandings) {
    ...
    p <- ggplot2::ggplot(data = scaled) +
      ggplot2::geom_line(
        mapping = ggplot2::aes(x = YEAR, y = SCALEDINDEX),
        linewidth = 1,
        color = "black"
      ) +
    ...
    scalingFactorPPR <- max(scaled$landings / 10^6) /
      max(scaled$SCALEDINDEX, na.rm = T)
    p <- ggplot2::ggplot(data = scaled) +
      ggplot2::geom_line(
        mapping = ggplot2::aes(x = YEAR, y = landings / 10^6),
        linewidth = 1,
        color = "grey"
      ) +
      ggplot2::geom_line(
        mapping = ggplot2::aes(x = YEAR, y = SCALEDINDEX * scalingFactorPPR),
        size = 1,
        color = "black"
      ) +
      ggplot2::scale_y_continuous(
        name = "Landings (Metric Tons)",
        sec.axis = ggplot2::sec_axis(
          ~ (. / scalingFactorPPR),
          name = expression(paste("Proportion of Total Primary Production"))
        )
      ) +
      ggplot2::theme(
